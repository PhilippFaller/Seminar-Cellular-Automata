...
\begin{definition}[Radius und Durchmesser]
	...
	Der \defWord{Abstand zwischen zwei Knoten $p, q$ in $\Gamma$}, geschrieben $\text{dist}(p , q)$, ist die Länge der kürzesten Wege von $p$ nach $q$. Damit heißt
	\begin{displaymath}
	r(k) := \max_{\mathclap{f(m) \neq \#}} \text{ dist}(k, m)
	\end{displaymath}
...
\begin{beispiel}
	In der folgenden Abbildung ist jede Zeile eine Konfiguration des längsten Astes. $S$ ist wieder grün dargestellt, genauso wie $S'$. Die linke Ziffer im Knoten ist das Carrybit und die rechte das Zählerbit. $\hat{n}$ ist der doppelt umrahmte Knoten.
	\par\medskip \noindent
	\begin{minipage}{\textwidth}
		\centering
		\begin{tikzpicture}[node distance=2cm, baseline=(current bounding box.north)]
