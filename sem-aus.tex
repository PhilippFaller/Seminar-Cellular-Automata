...
Eine Zustandsüberführungsfunktion $\delta$, für den oben beschriebenen Algorithmus könnte wie folgt aussehen: 
...
$N$ ist dabei ein neutrales Zustand, in dem sich ein Knoten befindet, wenn er noch kein anderes Signal erhalten hat und $W$ der Zustand, in dem ein Knoten auf das Signal $S'$ von seinen Nachfolgern wartet. 
...
\begin{beispiel}
	In der folgenden Abbildung ist jede Zeile der Längste Ast zu einem Zeitpunkt. $S$ ist wieder grün dargestellt. Die linke Ziffer im Knoten ist das Carrybit und die rechte das Zählerbit.
	\par\medskip \noindent
	\begin{minipage}{\textwidth}
		\centering
		\begin{tikzpicture}[node distance=2cm, baseline=(current bounding box.north)]
