...
In jedem Zeitschritt liest $M_n$ die Zustände seiner Nachbarn, sowie den Nachbarschaftsvektor und wechselt dann, wie durch die Zustandsübergangsfunktion $\delta$ festgelegt, in einen neuen Zustand. 
...
\begin{definition}[Konfiguration]
	...
	Es sei $n$ ein Knoten mit Zustand $q = c(n)$ und Nachbarn $m_1 := h(n, 1), \dots, m_d := h(n, d)$. 
	Es sei $q = \left( c(m_1), \dots , c(m_d)\right) \in Q^d$ der Vektor, der die Zustände der Nachbarn von $n$ enthält. 
...
\begin{definition}[Radius und Durchmesser]
	Es sei $\Gamma$ ein zusammenhängender $d$"~Graph und $k$ ein Knoten von $\Gamma$ mit $f(k) \neq \#$.
	Der \defWord{Abstand zwischen zwei Knoten $m, n$ in $\Gamma$}, geschrieben $\text{dist}(m , n)$, ist die Länge der kürzesten Wege von $m$ nach $n$. Damit heißt
	\begin{displaymath}
	r(k) := \max_{\mathclap{f(l) \neq \#}} \text{ dist}(k, l)
	\end{displaymath}
	der \defWord{Radius von $\Gamma$ zum Zentrum $k$}.
	Es sei $\hat{n}$ der ausgezeichnete Knoten in $\Gamma$.
	Dann heißt $r := r(\hat{n})$ der \defWord{Radius von $\Gamma$}.
	Weiter definieren wir, dass
	\begin{displaymath}
		R(\Gamma) := \min_{f(n) \neq \#} r(n)
	\end{displaymath}
	der \defWord{echte Radius von $\Gamma$} heißt.
...
\begin{beispiel} 
	...
	Der ausgezeichnete Knoten ist doppelt umrandet.
...
Ein Knoten $l$, der das Blatt auf einem der längsten Äste ist, ist einer der am weitesten von $k$ entfernten Knoten, da unser Spannbaum aus kürzesten Wegen besteht. 
Es gilt also:
\begin{displaymath}
h = \max_{\mathclap{f(l) \neq \#}} \text{ dist}(k, l) = r(k)
\end{displaymath}
Um $l$ zu bestimmen gehen wir wie folgt vor:
...
Es ist klar, dass das Signal $S_2$ vom weitesten entfernten Blatt $l$ zuletzt bei $k$ ankommt. 
Damit sind also die einzigen Knoten, die nicht $R$ erhalten haben, diejenigen, die auf dem Ast von $k$ nach $l$ liegen.
$k$ kann nun ein Signal entlang dieses Astes schicken, damit die Knoten wissen, dass sie Teil des längsten Astes sind.
Das benötigt zweimal $r(k)$ Zeit. Soll $k$ zusätzlich wissen, wann das Markieren beendet ist, sendet $l$ ein Signal zu $k$ zurück. Das benötigt noch einmal $r(k)$ Schritte.
...
Zunächst sendet $k$ ein Signal $S$ aus. Dieses Signal wird von $l$ wieder zu $k$ zurückgeschickt. Das benötigt $2r(k)$ Zeitschritte. Wir zählen also mit halber Geschwindigkeit die Schritte, die $S$ benötigt, um wieder zu $k$ zurückzukehren.

Das Zählen funktioniert nach dem Prinzip des Carry-Ripple-Addierers. 
Das niedrigstwertige Bit ist dabei in $k$ und das höchstwertige Bit in $l$ zu finden.
...
Damit $k$ feststellen kann, ob dieser Vorgang zu Ende ist, kann $l$ zusätzlich zur Reflektion von $S$ ein Signal $S'$ senden, dass mit halber Geschwindigkeit übertragen, also von jedem Knoten erst nach einem Zeitschritt Pause weitergeleitet wird.
...
Wir wollen abschließend herausfinden, ob eine bestimmte Beschriftung $\alpha$ in unserem $d$-Graph vorhanden ist. Dazu sei wieder der Spannbaum aus dem vorletzten Abschnitt gegeben. Ein Knoten $l$, der einen längsten Pfad zu $k$ hat, kann, wie im letzten Abschnitt beschrieben, gefunden werden. $k$ sendet nun ein Signal $S$ aus, das an alle Knoten weitergeleitet wird. Erhält ein Knoten mit Beschriftung $\alpha$ dieses Signal, so sendet er ein Signal $S_m$ zurück, um anzuzeigen, dass $\alpha$ gefunden wurde. $S_m$ soll dabei ein dominierendes Signal sein. Das heißt, wenn ein Knoten mehrere Signale neben $S_m$ erhält, leitet er lediglich $S_m$ weiter. Hat $l$ nicht die Beschriftung $\alpha$ und erhält das Signal $S$, sendet $l$ ein weiteres Signal $S_r$ aus. Erreicht $S_r$ den Knoten $k$, ohne dass zuvor ein $S_m$ eingegangen ist, weiß $k$, dass es die Beschriftung $\alpha$ nicht gibt. Denn wäre das der Fall, hätte der so beschriftete Knoten $S_m$ losgeschickt und da $l$ einen längsten Weg zu $k$ hat, wäre dieses $S_m$ spätestens gleichzeitig mit $S_r$ bei $k$ angekommen.
%-----------------------------------------------------------------------------
\printbibliography
\end{document}
%=============================================================================
